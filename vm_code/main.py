...
def preprocess():
    status = helper_functions.run_preprocessing(app.config['BUCKET_NAME'], app.config['REGION'])
    if status == 'DOCKER_IMAGE_ERROR':
        message = 'Docker image is not ready for preprocessing. Try once more in a minute!'
...
def job_submission():
    print(request.form)
    status = helper_functions.submit_job(request, app.config['BUCKET_NAME'], app.config['REGION'])
    if status == 'DOCKER_IMAGE_ERROR':
        message = 'Docker image is not ready for training. Try once more in a minute!'
    elif status == 'JOB_SUBMITTED':
